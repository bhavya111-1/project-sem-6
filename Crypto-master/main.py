...
def insertintotable():
    ...
    def LSTM_ALGO(df):
        ...
        if quote=="SOL1-USD":
            ...
            data = [4741, 3267, 6674]
        if quote=="LUNA-USD":
            model = load_model('static/TERRA.h5')
            data = [3468, 994, 6827]
        if quote=="USDC-USD":
            model = load_model('static/USD.h5')
            data = [1040, 1409, 3531]
        if quote=="AVAX-USD":
            model = load_model('static/Avalanche.h5')
            data = [6602, 1171, 9269]
